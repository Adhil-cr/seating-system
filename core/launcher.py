...
BUNDLE_DIR = Path(sys._MEIPASS).resolve() if FROZEN else APP_DIR.parent
...
os.environ["RUNTIME_DATA_ROOT"] = str(RUNTIME_DATA_ROOT)
...
def validate_bundle(settings):
    ...
    if missing_paths:
        missing = ", ".join(str(path) for path in missing_paths)
        raise RuntimeError(f"ExamCell bundle is incomplete. Missing: {missing}")
...
def open_browser_when_ready():
    ...
    print(f"ExamCell server did not become reachable at {URL}", flush=True)
...
def print_runtime_config(settings):
    ...
    print(f"DATABASE = {settings.DATABASES['default']['NAME']}", flush=True)
    print("=" * 60, flush=True)
...
def main():
    ...
    if not db_file.exists():
        print("First launch detected. Creating SQLite database...", flush=True)
    else:
